/* ==========================================================================
   Vitrine — theming
   --------------------------------------------------------------------------
   White-label in four tokens: accent, radius, font, logo.
...
(function (global) {
  ...
  const PRESETS = {
    ...
    // Deliberately included: a brand colour that fails contrast out of the
    // box. Pick it in the demo and watch the gate correct it rather than
    // ship an unreadable widget.
    citrus: {
  ...
  function apply(root, theme, scheme) {
    ...
    const surface = isDark ? { r: 16, g: 16, b: 18 } : WHITE;
    ...
    if (!accent) {
      ...
      accent = hexToRgb(PRESETS.graphite.accent);
    }
    ...
    // Foreground on the accent fill is chosen, never assumed. A yellow button
    // gets black text; a navy button gets white.
    ...
    report.push({
      ...
      detail: (onAccent === WHITE ? "white" : "black") + " chosen, " + contrast(accent, onAccent).toFixed(2) + ":1"
    });
    ...
    report.push({ check: "tenant surface", status: "PASS", detail: "4 tokens written; layout, spacing, depth and motion stay host-owned" });

    return {
      applied: { accent: rgbToHex(accent), accentText: rgbToHex(fixed.color), onAccent: rgbToHex(onAccent), radius: clamped },
      report: report
    };
  }
